...
class StackParticle : public StackObject
{
public:
    ...
    void setup(float _width, float _height)
    {
        ...
        billboards.getNormals().resize(MAX_PARTICLE,0);
        int _scale = 32;
        for(int i = 0 ; i < MAX_PARTICLE ; i++)
        {
            pos[i] = ofPoint(ofRandom(0,width),height*0.5);//ofRandomf()*width,ofRandomf()*height);
            acc[i] = 0;//ofPoint(ofRandomf()*0.01,ofRandomf()*0.01,0);
            vel[i] = 0;//ofPoint(ofRandom(-1,1),ofRandom(-1,1),0);
            per[i] = 0;
            fic[i] = ofRandom(0.98f,0.99f);
            age[i] = 100;
            decay[i]  = (abs(i-MAX_PARTICLE*0.5f)/MAX_PARTICLE*0.5f)*0.7;//ofRandom(10,50);
            
            scale[i]  = _scale;//ofRandom(_scale/2,_scale);
        ...
        for(int p = 0 ; p < MAX_PARTICLE;  p++)
        {
            ...
            if(age[p]>0)
            {
                ...
                if(isCompleted())
                {
                    ...
                    pos[p].x -= (pos[p].x-(((width/MAX_PARTICLE*1.0f)*p)))*0.1f;
